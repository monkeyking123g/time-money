...
const SingIn = ({ handleSingUp, setCredensial}) => {
    ...
    const [stateError, setStateError] = useState({error: false, title: ""})
   ...
   const handleTest = () => {
    ...
    const handleSubmit = (event) => {
      ...
      Axios.get("http://localhost:3002/api/get/user").then((data)=>{
        ...
        if (user.email === email && user.password === password) {
          ...
          setStateError({error: true, title : `This password incorrect " ${password} " !`});
        }else if(user.email !== email && user.password === password){
          //console.log("email non e correta")
          setStateError({error: true, title : `This email incorrect " ${email} " !`});
        }else if(user.email !== email && user.password !== password) {
          setStateError({error: true, title : "Email and password incorrect !"});
        }
        });
      });
    ...
    return (
        <Box component="form" noValidate onSubmit={handleSubmit} mt={1} sx={CustomInputGlobaol.root}>
        <CustomizedSnackbars  SnackbarOpen={stateError} setSnackbarOpen={setStateError}/>  
        <TextField
          margin="normal"
          required
          fullWidth
